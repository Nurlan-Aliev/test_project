...
- sanic - веб фреймворк(рекомендуемый, допускается альтернативный веб фреймворкфрейморк НО НЕ DJANGO)
- docker compose
...
### При обработке веб хукавебхука необходимо:
...
Транзакции являются уникальными, начисление суммы с одним transaction idtransaction_id должно производиться только один раз.
